...
class callspam:
	def main(self):
		...
		parser.add_argument('phonenum', nargs="?", metavar='phone', help='Телефонный номер жертвы (пример: 79991231122)')
		parser.add_argument('--name', nargs="?", help='Имя жертвы (по умолчанию: Саша)')
		parser.add_argument('--text', nargs="?", help='Текст с жалобой (по умолчанию: сомнительная компания вымогает деньги)')
		...
		_name = args.name
		_text = args.text
		_phone = '79384411045' #args.phonenum
		if _phone[0] == '+':
			_phone = _phone[1:]
		if _phone[0] == '8':
			_phone = '7'+_phone[1:]
		...
		if _name == None:
			_name = 'Леонид'
		if _text == None:
			_text = 'Здравствуйте! Перезвоните, пожалуйста'
